export const userFilter = (filter, x, y) => {
  if (filter === 'Sort by Pending') {
    ...
    if (x.uid && y.uid) {
      if (x.criteriaMet && x.criteriaMet != y.criteriaMet) {
        return -1
      } else if (y.criteriaMet && x.criteriaMet != y.criteriaMet) {
        return 1
      } else {
        return 0
      }
...
export const userMatch = (searchQuery, user) => {
  ...
  var match = true
  if (searchQuery.uid) {
    if (!(user.uid && user.uid.includes(searchQuery.uid))) {
      uidMatch = false
    }
  }
  ...
  if (!uidMatch || !emailMatch || !nameMatch) {
    match = false
  }
  return match
}
